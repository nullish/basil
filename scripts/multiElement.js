...
const basilMultiElement = async (args) => {
  ...
  console.log(
    "Scraping " +
      ...
      " pages for shuspace links, in batches of " +
      parallel,
  );
        ...
        promises.push(
          browser.newPage().then(async (page) => {
            ...
            try {
              ...
              const lnx = await page.$$eval(confEl, (as) =>
                as.map((a) => [a.innerText.replace(/\n/g, "||"), a.href]),
              );
              let arrOut = await lnx.map((e) => [
                timeStamp,
                arrUniquePages[elem],
                e[0].trim(),
                e[1],
              ...
              if (strOut.length == 0) {
                fs.appendFileSync(
                  outPath,
                  `"${timeStamp}","${arrUniquePages[elem]}","ELEMENT NOT FOUND",""\n`,
                );
              }
            } catch (err) {
