...
const basilFindTextAnywhere = async (args) => {
  ...
  console.log(
    "Scraping " +
      arrUniquePages.length +
      " pages for text pattern , in batches of " +
      parallel,
  );
  ...
  fs.appendFileSync(
    outPath,
    '"timestamp","batch","index","URL","Matches","Error"',
  );
        ...
        promises.push(
          browser.newPage().then(async (page) => {
            ...
            try {
              ...
              fs.appendFileSync(
                outPath,
                `"${timeStamp}","${k}","${j}","${arrUniquePages[elem]}","${bodyHTML.match(rx) === null ? 0 : bodyHTML.match(rx).length}",""\n`,
              );
            } catch (err) {
